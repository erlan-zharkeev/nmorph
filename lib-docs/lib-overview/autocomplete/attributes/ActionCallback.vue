...
<template>
  <div id="content-action-callback">
    <Attribute header="Action-callback" :subtitle="$t('overview.autocomplete.action-callback.subtitle')"
      :codeToCopy="code">
      <template #overview>
        <div class="autocomplete-action-callback-overview">
          <ClientOnly>
            <NmorphAutocomplete v-model="text" :list="list"
              :placeholder="$t('overview.autocomplete.basic-usage.placeholder')" :action-callback="actionCallback" />
          </ClientOnly>
        </div>
      </template>
      <template #code>
