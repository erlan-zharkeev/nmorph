...
<template>
  <div id="content-basic-usage">
    <Attribute :header="$t('overview.basic-usage')" :codeToCopy="code" info-name="alert.basic-usage">
      <template #overview>
        <div class="alert-basic-usage-overview">
          <ClientOnly>
            <div class="first-row">
              <div v-for="(alert, idx) in alerts" :key="idx">
                <NmorphAlert v-if="!alert.closed" :content="$t('overview.alert.basic-usage.content')"
                  :title="$t('overview.alert.basic-usage.title')" v-bind="alert"
                  @close="() => closeHandler(alert.id)" />
              </div>
            </div>
            <div class="second-row">
              <NmorphAlert
                html="<span>Neque porro quisquam est qui dolorem ipsum quia dolor sit amet, consectetur, adipisci velit...</span><br><p>Lorem ipsum</p>"
                fill :bordered="false" :show-icon="false" />
            </div>
          </ClientOnly>
        </div>
      </template>
